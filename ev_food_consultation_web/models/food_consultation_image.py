...
class FooDConsultationImage(models.Model):
    _name = 'food.consultation.image'
    _description = "Food Consultation Image"
    ...
    product_tmpl_id = fields.Many2one('food.consultation.detail', "Product Template", index=True, ondelete='cascade')
    ...
    #         if vals.get('product_tmpl_id') and 'default_product_tmpl_id' in self.env.context:
    #             variant_vals_list.append(vals)
    #         else:
    #             normal_vals.append(vals)
    #
    #     return super().create(normal_vals) + super(FooDConsultationImage, context_without_template).create(variant_vals_list)
